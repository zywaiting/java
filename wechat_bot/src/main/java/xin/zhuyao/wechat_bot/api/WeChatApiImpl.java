package xin.zhuyao.wechat_bot.api;
...
import xin.zhuyao.wechat_bot.api.constant.Constant;
import xin.zhuyao.wechat_bot.api.constant.StateCode;
import xin.zhuyao.wechat_bot.api.enums.AccountType;
import xin.zhuyao.wechat_bot.api.enums.ApiURL;
import xin.zhuyao.wechat_bot.api.enums.RetCode;
import xin.zhuyao.wechat_bot.api.model.*;
import xin.zhuyao.wechat_bot.api.request.BaseRequest;
import xin.zhuyao.wechat_bot.api.request.FileRequest;
import xin.zhuyao.wechat_bot.api.request.JsonRequest;
import xin.zhuyao.wechat_bot.api.request.StringRequest;
import xin.zhuyao.wechat_bot.api.response.*;
import xin.zhuyao.wechat_bot.exception.WeChatException;
import xin.zhuyao.wechat_bot.utils.*;
...
import static xin.zhuyao.wechat_bot.api.constant.Constant.*;
...
public class WeChatApiImpl implements WeChatApi {
     ...
     * 自动登录
     */
    private void autoLogin(String uuidStr) {
        ...
        try {
            ...
            this.login(false,uuidStr);
        }
    }

    @Override
    public void login(boolean autoLogin, String uuidStr) {
        if (bot.isRunning() || logging) {
            log.warn("微信已经登录");
            return;
        }
        if (autoLogin) {
            this.autoLogin(uuidStr);
        } else {
            this.logging = true;
            while (logging) {
                this.uuid = pushLogin();
                if (null == this.uuid) {
                    ...
                    log.info("开始下载二维码");
                    this.getQrImage(this.uuid, uuidStr);
     ...
     * @param uuidStr 保存路径
     */
    private String getQrImage(String uuid, String uuidStr) {
        ...
        try {
            loginPngUrl = OssUploadFileUtils.OssUploadFileInputStreamtest("wq-zy",
                    "wechatBot/login/" + uuidStr + "." + "png",
                    inputStream);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
